...
`GROUP_ID` оставлен для совместимости с одной группой. Для нескольких групп заполни `GROUP_IDS` ID групп через запятую. `ALLOWED_GROUP_IDS` — необязательный алиас для тех же значений. Если указано несколько параметров, бот объединит все ID без дублей.

**Как узнать GROUP_ID / GROUP_IDS без helper-ботов:**

Подробная ручная инструкция есть в **[local.md](./local.md)**. Коротко: добавь своего бота в группу, отправь новое сообщение в группе и получи `chat.id` через Telegram Bot API:
...
В ответе найди `chat.id` у объекта с `type: "group"` или `type: "supergroup"`, например `-1009876543210`. Для нескольких групп повтори шаги и запиши ID через запятую в `GROUP_IDS`.
